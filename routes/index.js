...
router.post('/login',function(req,res,next){
  ...
  async function funs(){
    ...
    var query3 = 'delete from notice where notice < date_add(NOW(),interval -3 day)';
...
router.post('/createShare',function(req,res,next){
  ...
  console.log(position,positionA,positionB,beginTime,endTime,renderID,cost);
...
router.post('/share/:shareID/windup',function(req,res,next){
  ...
  async function funs(){
    ...
    if(stars>2){
      let docs = [query1,query2,query3,query4];
    }else if(stars == 1){
      let docs = [query5,query2,query3,query4];
    }else{
...
router.get('/baduser/:baduserID',function(req,res,next){
  ...
  //}else{
    ...
    async function funs(){
      var query1 = 'select balance from user where userID = '+userID;
      ...
      res.render('baduser',{userID:userID,balance:resulets[0][0].balance,rendNum:resulets[1][0]["count(*)"],borrowNum:resulets[2][0]["count(*)"],rendRate:rendRate,borrowRate:borrowRate});
    }
    funs();
  //}
})
